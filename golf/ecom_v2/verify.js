...
function verifyInput(form){
    ...
    if (emptyFields === 0 && isValidEmail) {
      $('#form').submit();
    } else {
      $("#status_text").html("Missing Fields: </br>");
      $('input.required').each(function(index,element){

        if(element.value===""){
          if(element.name == 'cc_name'){
            $("#status_text").append("Name</br>");
          }else{
            $("#status_text").append(element.name.split("_").join(" ")+"</br>");
          }
        }
      })
      if(isValidEmail === false){
        $("#status_text").append("Email Address not valid");
      }
      return false;
    }
}
